...
function(torchdist_add_target target)
    ...
    if(TORCHDIST_ENABLE_ASAN OR TORCHDIST_ENABLE_UBSAN OR TORCHDIST_ENABLE_TSAN)
        target_compile_options(${target}
            PRIVATE
                -fno-omit-frame-pointer
        )

        if(TORCHDIST_ENABLE_ASAN)
            list(APPEND sanitizers -fsanitize=address)
        endif()

        if(TORCHDIST_ENABLE_UBSAN)
            list(APPEND sanitizers -fsanitize=undefined)
        endif()

        if(TORCHDIST_ENABLE_TSAN)
        ...
        if(TORCHDIST_ENABLE_ASAN AND CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
            target_compile_definitions(${target}
                PRIVATE
